...
const LogFilters = ({ values = {}, handleSubmit, handleReset }: LogFiltersProps) => {
  ...
  const hasActiveFilters = (values.id && values.id.trim() !== '') || values.reviewed_at !== undefined
  ...
  return (
    <Form key={formKey} fields={['id', 'reviewed_at']} className="mb-6">
      <div className="flex flex-wrap items-center gap-2 rounded bg-gray-100 p-2">
        <div className="max-sm:flex-1 sm:w-5/12 md:w-4/12">
          <Input name="id" placeholder={translations.findById} initialValue={values.id} noBottomGutter />
        </div>
        {enableVerification && (
