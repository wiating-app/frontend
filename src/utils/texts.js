export default {
  pl: {
    yes: 'Tak',
    no: 'Nie',
    is: 'Jest',
    none: 'Brak',
    save: 'Zapisz',
    cancel: 'Anuluj',
    loading: 'Ładowanie',
    search: 'Szukaj',
    backToResults: 'Powrót do wyników',
    locationInfo: {
      description: 'Opis',
      directions: 'Wskazówki dojścia',
      fireAccess: 'Dostęp do ognia',
      waterAccess: 'Dostęp do wody',
      lastUpdate: 'Ostatnia aktualizacja',
    },
    locationType: {
      hut: 'Chatka',
      hutFireplace: 'Chatka z paleniskiem',
      shed: 'Wiata',
      waterSource: 'Źródło',
      cave: 'Jaskinia',
      pasture: 'Paśnik',
      raisedHide: 'Ambona',
      tower: 'Wieża widokowa',
    },
    noResults: 'Nic nie znaleziono',
    connectionProblem: {
      map: 'Nie można pobrać markerów. Problem z połączeniem.',
      location: 'Nie udało się pobrać danych lokalizacji. Problem z połączeniem.',
    },
    markerForm: {
      heading: {
        addMarker: 'Dodaj nowe miejsce',
        editMarker: 'Edytuj miejsce',
      },
      location: 'Położenie geograficzne',
      locationHelp: 'Wprowadź współrzędne w którymkolwiek popularnym formacie, a zostaną automatycznie przekonwertowane do systemu decymalnego.',
      place: 'Nazwa miejsca',
      type: 'Typ miejsca',
      description: 'Opis miejsca',
      waterDescription: 'Opis dostępu do wody (opcjonalnie)',
      fireDescription: 'Opis dostępu do ognia (opcjonalnie)',
    },
    notifications: {
      markerUpdated: 'Marker zaktualizowany.',
      newMarkerAdded: 'Dodano nowy marker.',
      couldNotSaveMarker: 'Nie udało się zapisać markera.',
      photoAdded: 'Dodano nowe zdjęcie.',
      couldNotSavePhoto: 'Nie udało się zapisać zdjęcia.',
      wrongCoordsFormat: 'Niepoprawny format współrzędnych.',
    },
    actions: {
      edit: 'Edytuj',
      addPhoto: 'Dodaj zdjęcie',
    },
    addMarker: 'Dodaj miejsce',
    auth: {
      loginSuccessful: 'Zalogowano pomyślnie',
      login: 'Logowanie',
      logout: 'Wyloguj',
    },
  },

  en: {
    yes: 'Yes',
    no: 'No',
    is: 'Is available',
    none: 'None',
    save: 'Save',
    cancel: 'Cancel',
    loading: 'Loading',
    search: 'Search',
    backToResults: 'Back to results',
    locationInfo: {
      description: 'Description',
      directions: 'How to reach the place',
      fireAccess: 'Access to fire',
      waterAccess: 'Access to water',
      lastUpdate: 'Last update',
    },
    locationType: {
      hut: 'Hut',
      hutFireplace: 'Hut with a fireplace',
      shed: 'Shed',
      waterSource: 'Water source',
      cave: 'Cave',
      pasture: 'Pasture',
      raisedHide: 'Raised Hide',
      tower: 'Observation Tower',
    },
    noResults: 'No results',
    connectionProblem: {
      map: 'Could not load markers. Connection problem.',
      location: 'Could not load location details. Connection problem.',
    },
    markerForm: {
      heading: {
        addMarker: 'Add new place',
        editMarker: 'Edit place',
      },
      location: 'Coordinates',
      locationHelp: 'Enter coordinates in any of the popular format. They will be converted to decimal system automatically.',
      place: 'Name of the place',
      type: 'Type of the place',
      description: 'Description',
      waterDescription: 'Access to water description (optional)',
      fireDescription: 'Access to fire description (optional)',
    },
    notifications: {
      markerUpdated: 'Marker updated.',
      newMarkerAdded: 'New marker added.',
      couldNotSaveMarker: 'Could not save a marker.',
      photoAdded: 'Photo added.',
      couldNotSavePhoto: 'Could not save a photo.',
      wrongCoordsFormat: 'Wrong coordinations format.',
    },
    actions: {
      edit: 'Edit',
      addPhoto: 'Add photo',
    },
    addMarker: 'Add place',
    auth: {
      loginSuccessful: 'Login successful',
      login: 'Login',
      logout: 'Logout',
    },
  },
}
