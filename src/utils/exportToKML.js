...
const exportToKML = locations => {
  ...
  try {
    ...
    const placemarks = locations.reduce((acc, { _source }) => [
      ...
      `          <p><strong>Dostęp do wody:</strong> ${!_source.water || !_source.water.exists ? 'brak.' : _source.water.comment || 'jest.'}</p>`,
      `          <p><strong>Dostęp do ognia:</strong> ${!_source.fire || !_source.fire.exists ? 'brak.' : _source.fire.comment || 'jest.'}</p>`,
      '        ]]>',
      '      </description>',
      '      <Point>',
